...
- `assets/css/main.css` — Tailwind v4 source. The compiled output `assets/css/compiled/main.css` is committed to the repository and regenerated by CI on every build. Link background treatments live under `@layer components` as `.link-<type>`.
