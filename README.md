...
- `engines.node` in `package.json` — Node version
...
- `assets/css/main.css` — Tailwind v4 source. The compiled output `assets/css/compiled/main.css` is committed to the repository so CI (which does not run the Tailwind build) can ship a complete site. Link background treatments live under `@layer components` as `.link-<type>`.
...
At the moment, these workflows invoke `hugo --environment production --minify` for CI/release builds. If you change them, make sure the pipeline still builds the Tailwind CSS before publishing artifacts; otherwise deploys/releases can ship without the compiled `.link-<type>` styles.
