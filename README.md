...
- `assets/css/main.css` — Tailwind v4 source. The compiled output in `assets/css/compiled/` is git-ignored. Link background treatments live under `@layer components` as `.link-<type>`.
...
Required build order in both: `npm ci` → `npm run build:css` → `hugo --environment production --minify`.
